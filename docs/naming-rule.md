...
このドキュメントは、ユーティリティ関数やファイル名における命名規則を整理したもの。命名の一貫性を保つことで、コードの可読性・保守性を高めることを目的とする。
...
| `is◯◯`        | `isValidEmail(val)`     | 真偽値を返す判定関数           |
| `has◯◯`       | `hasPermission(user)`   | 真偽値を返す判定関数           |
| `can◯◯`       | `canAccess(user)`       | 真偽値を返す判定関数           |
